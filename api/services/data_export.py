...
class DataExportService:
    ...
    async def export_session_data(
        ...
        filtered_history = self._filter_history_by_time(
            history, start_time, end_time
        )
        
        # Filter by variables if specified
        if variables:
            filtered_history = self._filter_history_by_variables(
                filtered_history, variables
            )
    ...
    def _filter_history_by_time(
        ...
        if start_time is not None:
            start_idx = next(
                (i for i, t in enumerate(times) if t >= start_time),
                0
            )
        
        if end_time is not None:
            end_idx = next(
                (i for i, t in enumerate(times) if t > end_time),
                len(times)
            )
    ...
    def _compute_energy_stats(
        ...
        if free_energy:
            stats["mean_free_energy"] = round(
                sum(free_energy) / len(free_energy), 3
            )
    ...
    def _compute_allostasis_stats(
        ...
        if not allostatic_load:
            return {
                ...
                "max_allostatic_load": 0.0
            }
        
        return {
            "mean_allostatic_load": round(
                sum(allostatic_load) / len(allostatic_load), 3
            ),
