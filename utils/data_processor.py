        ...
        for col in data.columns:
            if col == "timestamp":
                if not pd.api.types.is_datetime64_any_dtype(data[col]):
                    validation_results["warnings"].append(f"Column '{col}' should be datetime type")
    ...
    def extract_features(
        ...
        for start_idx in range(0, len(data) - window_size + 1, step_size):
            ...
            feature_row = {
                ...
                "window_end": end_idx,
                "timestamp_start": (
                    data.iloc[start_idx]["timestamp"] if "timestamp" in data.columns else None
                ),
                "timestamp_end": (
                    data.iloc[end_idx - 1]["timestamp"] if "timestamp" in data.columns else None
                ),
            }
