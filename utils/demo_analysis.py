...
def main() -> None:
    ...
    if len(dynamics["free_energy"]) > 0:
        ...
        print(
            f"Free energy range:      [{np.min(dynamics['free_energy']):.2f}, "
    ...
    if len(dynamics["precision"]) > 0:
        ...
        print(
            f"Precision range:        [{np.min(dynamics['precision']):.2f}, "
    ...
    if len(dynamics["ignition_signal"]) > 0:
        ...
        print(
            f"Ignition signal range:  [{np.min(dynamics['ignition_signal']):.2f}, "
