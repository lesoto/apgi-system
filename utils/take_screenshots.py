...
class APGIScreenshotDocumentation:
    ...
    def _discover_gui_elements(self):
        ...
        try:
            test_screenshot = pyautogui.screenshot(
                region=(self.gui_window.left, self.gui_window.top,
                        self.gui_window.width, self.gui_window.height)
            )
        ...
        total_elements = len(self.button_locations) + len(self.tab_locations) + len(self.slider_locations)
        print(f"✅ Found {len(self.button_locations)} buttons, {len(self.tab_locations)} tabs, {len(self.slider_locations)} sliders")
        return True
    ...
    def _use_fallback_discovery(self):
        ...
        self.button_locations = {
            "start": {"x": control_x - 100, "y": control_y, "rect": (control_x - 100, control_y, 80, 30), "confidence": 0.5},
            "pause": {"x": control_x, "y": control_y, "rect": (control_x, control_y, 80, 30), "confidence": 0.5},
            "stop": {"x": control_x + 100, "y": control_y, "rect": (control_x + 100, control_y, 80, 30), "confidence": 0.5},
            "reset": {"x": control_x + 200, "y": control_y, "rect": (control_x + 200, control_y, 80, 30), "confidence": 0.5}
        }
        ...
        tab_names = [
            "neural_activity", "interoception", "system_metrics", 
            "self_model", "oscillations", "state_space"
        ]
        ...
        slider_names = [
            "ignition_threshold", "extero_precision", "intero_precision",
            "arousal", "stress", "activity", "learning_rate", "attention_gain"
        ]
        ...
        print(f"✅ Fallback mode: Estimated {len(self.button_locations)} buttons, {len(self.tab_locations)} tabs, {len(self.slider_locations)} sliders")
        ...
        try:
            ...
            for title in title_variations:
                try:
                    if hasattr(gw, 'getWindowsWithTitle'):
                        ...
                        if windows:
                            print(f"✅ Found window by title variation '{title}': {windows[0].title}")
                            return windows[0]
                except Exception:
                    continue
            ...
            try:
                if hasattr(gw, 'getAllWindows'):
                    ...
                    for window in all_windows:
                        if hasattr(window, 'title') and window.title and len(window.title.strip()) > 0:
                            title_lower = window.title.lower()
                            
                            # Skip obvious system windows
                            if any(skip in title_lower for skip in [
                                "desktop", "finder", "spotlight", "notification", 
                                "system preferences", "activity monitor", "terminal",
                                "visual studio code", "vscode", "chrome", "safari",
                                "firefox", "edge", "opera"
                            ]):
                                continue
                            ...
                            if any(keyword in title_lower for keyword in [
                                "apgi", "consciousness", "modeling", "framework", "python", "tk"
                            ]):
                                candidate_windows.append(window)
                                print(f"🎯 Candidate: {window.title} ({getattr(window, 'width', 'unknown')}x{getattr(window, 'height', 'unknown')})")
                    
                    # Sort candidates by size (larger windows are more likely to be our GUI)
                    if candidate_windows:
                        candidate_windows.sort(key=lambda w: getattr(w, 'width', 0) * getattr(w, 'height', 0), reverse=True)
            ...
            try:
                if hasattr(gw, 'getAllWindows'):
                    ...
                    large_windows = []
                    for window in all_windows:
                        if (hasattr(window, 'width') and hasattr(window, 'height') and 
                            hasattr(window, 'title') and window.title):
                            ...
                            if window.width > 1000 and window.height > 700:
                                ...
                                if not any(skip in title_lower for skip in [
                                    "visual studio", "vscode", "code", "terminal", 
                                    "browser", "chrome", "safari", "firefox"
                                ]):
                                    large_windows.append(window)
                                    print(f"🔍 Large window: {window.title} ({window.width}x{window.height})")
    ...
    def _discover_buttons(self, screenshot: Image.Image):
        ...
        try:
            ...
            for detection in button_detections:
                name = detection["name"]
                if name not in unique_buttons or detection["confidence"] > unique_buttons[name]["confidence"]:
            ...
            for name, detection in unique_buttons.items():
                ...
                print(f"    Found {name} button at ({detection['x']}, {detection['y']}) [confidence: {detection['confidence']:.1f}]")
        ...
        try:
            ...
            if not self.tab_locations and self.gui_window:
                ...
                for i, tab_name in enumerate(tab_names):
                    ...
                    print(f"    Estimated tab '{tab_name}' at ({start_x + i * (tab_width + 10)}, {start_y})")
    ...
    def _discover_sliders(self, screenshot: Image.Image):
        ...
        try:
            ...
            for i, slider_candidate in enumerate(slider_candidates[:len(slider_names)]):
                ...
                print(f"    Found slider '{slider_name}' at ({slider_candidate['x']}, {slider_candidate['y']})")
            ...
            if not self.slider_locations and self.gui_window:
                ...
                for i, slider_name in enumerate(slider_names):
                    ...
                    print(f"    Estimated slider '{slider_name}' at ({start_x + slider_width // 2}, {y_pos + slider_height // 2})")
    ...
    def _discover_menu_items(self):
        ...
        print(f"    Found {len(self.menu_items)} menu categories with {total_menu_items} total items")
    ...
    def _document_all_screens(self):
        ...
        try:
            # 1. Initial state
            self._take_screenshot("01_initial_state", "Initial GUI state - application just started")
    ...
    def _document_all_buttons(self):
        ...
        for button_name, button_info in self.button_locations.items():
            try:
                ...
                # Take screenshot
                self._take_screenshot(
                    f"03_button_{button_name}",
                    f"Button clicked: {button_name.title()}"
                )
    ...
    def _document_simulation_states(self):
        ...
        try:
            ...
            if "start" in self.button_locations:
                pyautogui.click(self.button_locations["start"]["x"], 
                              self.button_locations["start"]["y"])
                time.sleep(3)
                
                self._take_screenshot(
                    "06_simulation_running",
                    "Simulation running - active state"
                )
            
            # Pause simulation - check if button exists
            if "pause" in self.button_locations:
                pyautogui.click(self.button_locations["pause"]["x"],
                              self.button_locations["pause"]["y"])
                time.sleep(2)
                
                self._take_screenshot(
                    "07_simulation_paused",
                    "Simulation paused - inactive state"
                )
            ...
            if "pause" in self.button_locations:
                ...
                self._take_screenshot(
                    "08_simulation_resumed",
                    "Simulation resumed - active state"
                )
            
            # Stop simulation
            if "stop" in self.button_locations:
                pyautogui.click(self.button_locations["stop"]["x"],
                              self.button_locations["stop"]["y"])
                time.sleep(2)
                
                self._take_screenshot(
                    "09_simulation_stopped",
                    "Simulation stopped - terminated state"
                )
            
            # Reset simulation
            if "reset" in self.button_locations:
                pyautogui.click(self.button_locations["reset"]["x"],
                              self.button_locations["reset"]["y"])
                time.sleep(2)
                
                self._take_screenshot(
                    "10_simulation_reset",
                    "Simulation reset - initial state"
                )
    ...
    def _document_view_toggles(self):
        ...
        try:
            ...
            if view_menu and "items" in view_menu:
                toggle_items = ["Control Panel", "Neural Activity", "Interoception", "System Metrics"]
    ...
    def _document_status_bar_and_log(self):
        ...
        try:
            ...
            self._take_screenshot(
                "17_event_log",
                "Event Log Panel - showing system events and messages"
            )
        ...
        try:
            ...
            if self.gui_window:
                try:
                    ...
                    if (self.gui_window.left >= 0 and self.gui_window.top >= 0 and 
                        self.gui_window.width > 0 and self.gui_window.height > 0):
                        ...
                        print(f"    📸 Captured window region: {self.gui_window.left},{self.gui_window.top} {self.gui_window.width}x{self.gui_window.height}")
                    else:
            ...
            screenshot_info = {
                ...
                "window_info": {
                    ...
                    "geometry": f"{self.gui_window.width}x{self.gui_window.height}" if self.gui_window else "Unknown"
                } if self.gui_window else None
            }
                ...
                try:
                    ...
                    if active and hasattr(active, 'title'):
                        if active.title == self.gui_window.title:
                            ...
                            print(f"    ⚠️ Active window is: {active.title}, expected: {self.gui_window.title}")
                except Exception as e:
        ...
        try:
            ...
            if hasattr(gw, 'getAllWindows'):
                ...
                for i, window in enumerate(all_windows):
                    if (hasattr(window, 'title') and window.title and 
                        ...
                        if any(skip in title_lower for skip in [
                            "desktop", "finder", "spotlight", "notification", 
                            "system preferences", "activity monitor", "dock",
                            "menu bar", "trash"
                        ]):
                            continue
    ...
    def _generate_html_report(self) -> str:
        ...
        discovered_elements = element_stats["buttons"] + element_stats["tabs"] + element_stats["sliders"] + element_stats["menus"]
        discovery_rate = (discovered_elements / total_expected_elements * 100) if total_expected_elements > 0 else 0
        ...
        total_documented_screens = (
            ...
            len(self.menu_items) + total_menu_items +  # Menus and submenu items
            5 +  # Simulation states (running, paused, resumed, stopped, reset)
            element_stats["dialogs"] +  # Dialog windows
            element_stats["view_toggles"] + element_stats["zoom_controls"] +  # View controls
