...
from apgi_simulation.core.motor_planning import MotorPlanner
...
from apgi_simulation.validation import InputValidator
...
class HierarchicalGaussianFilter:
    ...
    def update(self, observation: FloatArray, dt: float = 0.001) -> Tuple[List[BeliefState], float]:
        ...
        if not isinstance(observation, np.ndarray):
            raise TypeError(f"observation must be a numpy array, got {type(observation).__name__}")
    ...
    def _map_down(self, from_level: int, state: FloatArray) -> FloatArray:
        ...
        # Check for NaN/Inf in projection matrix (defensive programming)
        if not np.all(np.isfinite(projection_matrix)):
            logger.warning(
                "_map_down: projection matrix contains NaN/Inf at level %d; returning zeros.",
                from_level,
            )
            return np.zeros((self.batch_size, target_dim))

        # Check for NaN/Inf in input state (defensive programming)
        if not np.all(np.isfinite(state)):
            logger.warning(
                "_map_down: input state contains NaN/Inf at level %d; returning zeros.", from_level
            )
            return np.zeros((self.batch_size, target_dim))
    ...
    def _project_up(self, from_level: int, state: FloatArray) -> FloatArray:
        ...
        if not np.all(np.isfinite(projection_matrix)):
            logger.warning(
                "_project_up: projection matrix contains NaN/Inf at level %d; returning zeros.",
                from_level,
            )
            return np.zeros((self.batch_size, target_dim))
        ...
        if not np.all(np.isfinite(state)):
            logger.warning(
                "_project_up: input state contains NaN/Inf at level %d; returning zeros.",
                from_level,
            )
            return np.zeros((self.batch_size, target_dim))
...
class ActiveInferenceEngine:
    ...
    def step(
        ...
        info = {
            ...
            "efe_components": planning_info.get("efe_components", {}),
        }
