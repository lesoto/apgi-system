...
class MonitoringIntegration:
    ...
    def connect_system(self, apgi_simulation: Any) -> None:
        """
        Connect APGI system to monitor.

        Args:
            apgi_simulation: APGI system instance to monitor
        """
        # Add monitoring callback to system
        if hasattr(apgi_simulation, "add_monitor_callback"):
            apgi_simulation.add_monitor_callback(self.monitor.update_data)
            self.is_connected = True
            print("APGI system connected to simple monitor")
        else:
            print("Warning: APGI system does not support monitoring callbacks")

    def disconnect_system(self, apgi_simulation: Any) -> None:
        """
        Disconnect APGI system from monitor.

        Args:
            apgi_simulation: APGI system instance to disconnect
        """
        if hasattr(apgi_simulation, "remove_monitor_callback"):
            apgi_simulation.remove_monitor_callback(self.monitor.update_data)
            self.is_connected = False
