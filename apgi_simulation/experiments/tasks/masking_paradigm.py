...
class MaskingParadigmTask:
    ...
    def run_trial(self, apgi_simulation: Any, trial: Trial) -> TrialResult:
        ...
        target_start_time = apgi_simulation.time

        # Phase 1: Present target stimulus
        target_steps = int(trial.target_duration_ms)
        for step in range(target_steps):
            state = apgi_simulation.step(trial.target_stimulus)
        ...
        if isi_duration > 0:
            ...
            for step in range(isi_steps):
                state = apgi_simulation.step(blank)
        ...
        for step in range(mask_steps):
            state = apgi_simulation.step(trial.mask_stimulus)
