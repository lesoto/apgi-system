...
@given(
    num_steps=st.integers(min_value=5, max_value=50),
...
def test_property_export_data_completeness(num_steps, observation):
    ...
    **Validates: Requirements 12.1**
    ...
    required_fields = [
        'time',  # Timestamps
        'ignition', 'workspace_active',  # Event markers
        'free_energy', 'extero_precision', 'intero_precision',  # Core subsystem states
        'metabolic_reserves', 'allostatic_load',  # Thermodynamic/interoception states
        'heart_rate', 'cortisol',  # Body states
        'gamma_power', 'beta_power',  # Neural states
...
@given(
    num_steps=st.integers(min_value=10, max_value=50),
    observation=observation_strategy()
)
...
@given(
    num_steps=st.integers(min_value=5, max_value=30),
    observation=observation_strategy()
)
    ...
    try:
        ...
        for i, row in enumerate(csv_data):
            assert len(row) == len(headers), \
                f"Row {i} must have same number of columns as headers"
        ...
        for i, row in enumerate(csv_data[1:], start=1):
            ...
            assert row_keys == first_row_keys, \
                f"Row {i} column order must match first row"
    ...
    **Validates: Requirements 12.4**
    ...
    try:
        ...
        for i, (original_entry, loaded_entry) in enumerate(zip(original_data, loaded_data)):
            ...
            assert set(original_entry.keys()) == set(loaded_entry.keys()), \
            ...
            for key in original_entry.keys():
                ...
                assert isinstance(original_value, (int, float, np.number)), \
                    f"Original value for '{key}' must be numeric"
                assert isinstance(loaded_value, (int, float)), \
                    f"Loaded value for '{key}' must be numeric"
                
                # Values should be approximately equal (allowing for float precision)
                assert np.isclose(float(original_value), float(loaded_value), rtol=1e-9), \
        ...
        for i, entry in enumerate(loaded_data):
            assert isinstance(entry, dict), \
                f"Entry {i} must be a dictionary after round-trip"
