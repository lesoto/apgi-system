...
class TestInteroceptionProperties:
    ...
    def test_property_homeostatic_bounds_invariant(self, simulation_steps, arousal_sequence, activity_sequence, stress_sequence):
        """
        ...
        **Validates: Requirements 4.1**
        ...
        min_length = min(len(arousal_sequence), len(activity_sequence), len(stress_sequence), simulation_steps)
        ...
        for i in range(min_length):
            ...
            assert 40 <= current_state['heart_rate'] <= 200, \
            ...
            assert 5 <= current_state['respiration'] <= 50, \
            ...
            assert 35.0 <= current_state['temperature'] <= 42.0, \
            ...
            assert 1.0 <= current_state['glucose'] <= 15.0, \
            ...
            assert 1 <= current_state['cortisol'] <= 50, \
    ...
    def test_property_allostatic_load_accumulation(self, body_states):
        ...
        **Validates: Requirements 4.2**
        ...
        if len(body_states) > 3:  # Only check for longer sequences
            assert final_load >= initial_load - 0.1, \
        ...
        for load in loads:
            assert 0.0 <= load <= 1.0, \
                f"Allostatic load out of bounds: {load}"
    ...
    def test_property_somatic_marker_storage_completeness(self, context_action_pairs, current_time):
        ...
        **Validates: Requirements 4.3**
        ...
        for marker in sm_system.markers:
            ...
            assert -1.0 <= marker.body_outcome <= 1.0, \
            ...
            assert marker.last_update_time >= 0, \
    ...
    def test_property_somatic_marker_retrieval_accuracy(self, context, action, body_outcome, 
                                                       retrieval_context, retrieval_action):
        """
        ...
        **Validates: Requirements 4.4**
        ...
        assert min_gain <= gain_exact <= max_gain, \
        ...
        assert abs(gain_exact - expected_gain) < 1e-6, \
        ...
        gain_different, found_different = sm_system.retrieve(retrieval_context_arr, retrieval_action_arr)
    ...
    @given(
        ...
        predicted_state=body_state_strategy()
    )
    def test_property_interoceptive_prediction_error_computation(self, body_state, predicted_state):
        ...
        **Validates: Requirements 4.5**
        ...
        assert prediction_error.shape == (5,), f"Prediction error should be 5D: {prediction_error.shape}"
        
        # All error values should be finite
        assert np.all(np.isfinite(prediction_error)), \
        ...
        assert error_magnitude < 100.0, \
            f"Prediction error magnitude too large: {error_magnitude}"
