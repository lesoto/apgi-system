    ...
    def test_property_iowa_gambling_learning(self):
        ...
        **Validates: Requirements 7.1**
        ...
        task = IowaGamblingTask(
            num_trials=40,
            deck_selection_strategy='participant_choice'
        )
        ...
        assert analysis['total_trials'] == 40, \
            f"Expected 40 trials, got {analysis['total_trials']}"
...
class TestMaskingParadigmProperties:
    ...
    def test_property_masking_soa_effect(self):
        ...
        **Validates: Requirements 7.2**
        ...
        assert analysis['total_trials'] == expected_trials, \
        ...
        assert len(soa_analysis) == len(sorted_soas), \
...
class TestTaskResultCompletenessProperties:
    ...
    def test_property_iowa_gambling_result_completeness(self):
        ...
        **Validates: Requirements 7.5**
        ...
        assert len(task.results) == num_trials, \
    ...
    def test_property_masking_result_completeness(self):
        ...
        **Validates: Requirements 7.5**
        ...
        assert len(analysis['by_soa']) == len(soas), \
            f"Should have data for all {len(soas)} SOAs"
        ...
        assert len(masking_curve) == len(soas), \
            f"Masking curve should have {len(soas)} points"
        ...
        assert len(task.results) == expected_trials, \
    ...
    def test_property_attentional_blink_result_completeness(self):
        ...
        **Validates: Requirements 7.5**
        ...
        lag_analysis = analysis['lag_analysis']
        assert len(lag_analysis) == len(lags), \
            f"Should have data for all {len(lags)} lags"
        ...
        for result in task.results:
            ...
            assert hasattr(result, 't1_detected')
            assert hasattr(result, 't2_detected')
            assert hasattr(result, 'blink_occurred')
