...
Consolidated from:
- test_analysis_engine.py (basic tests)
- test_analysis_engine_comprehensive.py (comprehensive coverage)
- test_analysis_engine_coverage.py (execution path coverage)
...
class TestExecutionPathCoverage:
    """Tests for execution path coverage using fixtures."""
    ...
    def test_initialization_with_fixture(self, engine, output_dir):
        """Test AnalysisEngine initialization using fixtures."""
        assert engine.output_dir == Path(output_dir)
        assert engine.output_dir.exists()
        assert "descriptive" in engine.analysis_functions

    def test_all_analysis_types_with_fixture(self, engine, sample_data):
        """Test all analysis types with comprehensive sample data."""
        # Test descriptive with plots
        result = engine.analyze(sample_data, "descriptive", generate_plots=True)
        assert isinstance(result, AnalysisResult)
        assert "age" in result.statistics
        assert len(result.plots) > 0
        assert (engine.output_dir / "results" / f"{result.analysis_id}_summary.json").exists()

        # Test comparative
        result = engine.analyze(sample_data, "comparative", generate_plots=False)
        assert isinstance(result, AnalysisResult)
    ...
    def test_list_and_get_summary_with_fixture(self, engine, sample_data):
        """Test listing analyses and retrieving summary with fixtures."""
        result = engine.analyze(sample_data, "descriptive", generate_plots=False)
