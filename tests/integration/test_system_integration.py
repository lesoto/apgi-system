...
class TestFullSystemStepExecution:
    ...
    def test_system_step_completes_successfully(self, apgi_framework, random_observation):
        """
        ...
        state = apgi_framework.step(random_observation)

        # Verify step completed and returned state
        assert state is not None
        assert isinstance(state, dict)

    def test_system_step_updates_all_subsystems(self, apgi_framework, random_observation):
        """
    ...
    def test_system_step_advances_time(self, apgi_framework, random_observation):
        """
        Test that system time advances correctly with each step.

        Validates: Requirements 8.4
        """
        initial_time = apgi_framework.time
        timestep = apgi_framework.timestep_ms
        ...
        for i in range(5):
            ...
            assert abs(apgi_framework.time - expected_time) < 1e-6

    def test_system_step_maintains_history(self, apgi_framework, random_observation):
        """
        ...
        for _ in range(num_steps):
            apgi_framework.step(random_observation)

        # Verify history was recorded
        assert len(apgi_framework.history["time"]) == num_steps
        assert len(apgi_framework.history["ignitions"]) == num_steps
        assert len(apgi_framework.history["free_energy"]) == num_steps
        assert len(apgi_framework.history["precision"]) == num_steps
        assert len(apgi_framework.history["metabolic_reserves"]) == num_steps

    def test_system_step_with_context(self, apgi_framework, random_observation):
        """
        ...
        state = apgi_framework.step(random_observation, context=context)
...
class TestSubsystemUpdateOrdering:
    ...
    def test_body_model_updates_before_allostasis(self, apgi_framework, random_observation):
        """
    ...
    def test_precision_updates_before_ignition(self, apgi_framework, random_observation):
        """
    ...
    def test_ignition_updates_before_workspace(self, apgi_framework, random_observation):
        """
    ...
    def test_metabolism_updates_with_ignition_events(self, apgi_framework, random_observation):
        """
        Test that metabolic budget updates based on ignition events.

        Validates: Requirements 8.1, 8.4
        """
        initial_reserves = apgi_framework.metabolism.current_reserves
...
class TestStateConsistency:
    ...
    def test_timestamps_consistent_across_subsystems(self, apgi_framework, random_observation):
        """
        ...
        assert system_time == apgi_framework.time

    def test_state_dimensions_consistent(self, apgi_framework, random_observation):
        """
    ...
    def test_precision_values_within_valid_range(self, apgi_framework, random_observation):
        """
    ...
    def test_metabolic_reserves_within_bounds(self, apgi_framework, random_observation):
        """
        ...
        max_capacity = apgi_framework.metabolism.total_budget

        assert reserves >= 0, "Reserves should not be negative"
        assert reserves <= max_capacity, "Reserves should not exceed capacity"

    def test_body_state_physiologically_plausible(self, apgi_framework, random_observation):
        """
    ...
    def test_get_state_returns_complete_information(self, apgi_framework, random_observation):
        """
        ...
        full_state = apgi_framework.get_state()
...
class TestExtendedSimulation:
    ...
    def test_extended_simulation_completes(self, apgi_framework):
        """
        ...
        result = apgi_framework.run(duration_ms=duration_ms)

        assert result is not None
        assert result["total_steps"] > 0
        assert result["duration_ms"] == duration_ms

    def test_extended_simulation_maintains_numerical_stability(self, apgi_framework):
        """
    ...
    def test_extended_simulation_with_custom_input(self, apgi_framework):
        """
        ...
        duration_ms = 500.0
        result = apgi_framework.run(duration_ms=duration_ms, extero_input_fn=varying_input)

        assert result["total_steps"] > 0
        assert "final_state" in result

    def test_extended_simulation_tracks_ignition_events(self, apgi_framework):
        """
    ...
    def test_extended_simulation_performance(self, apgi_framework):
        """
        ...
        duration_ms = 500.0
        num_steps = int(duration_ms / apgi_framework.timestep_ms)
    ...
    def test_system_reset_after_extended_run(self, apgi_framework):
        """
        ...
        """
        # Run simulation
        apgi_framework.run(duration_ms=1000.0)

        # Record state before reset
        time_before = apgi_framework.time
        ...
        assert len(apgi_framework.history["time"]) == 0
        assert len(apgi_framework.history["ignitions"]) == 0

        # Verify system can run again after reset
        result = apgi_framework.run(duration_ms=500.0)
        assert result["total_steps"] > 0
...
class TestCausalRelationships:
    ...
    def test_ignition_depletes_metabolic_reserves(self, apgi_framework, random_observation):
        """
        ...
        if ignition_count > 0:
            final_reserves = apgi_framework.metabolism.current_reserves
            # Reserves should decrease or stay same (if recovery rate is high)
            assert final_reserves <= initial_reserves + (100 * apgi_framework.timestep_ms * 0.01)

    def test_allostatic_load_affects_ignition_threshold(self, apgi_framework, random_observation):
        """
        ...
        apgi_framework.ignition_threshold.current_threshold
        ...
        final_stats = apgi_framework.ignition_threshold.get_statistics()

        # Threshold should exist and be positive
        assert "mean_threshold" in final_stats
        assert final_stats["mean_threshold"] > 0
        assert apgi_framework.ignition_threshold.current_threshold > 0

    def test_somatic_markers_learned_from_experience(self, apgi_framework, random_observation):
        """
        Test that somatic markers are learned during simulation.

        Validates: Requirements 8.4
        """
        initial_markers = apgi_framework.somatic_markers.get_statistics()["num_markers"]

        # Run extended simulation to allow learning
        apgi_framework.run(duration_ms=2000.0)

        final_markers = apgi_framework.somatic_markers.get_statistics()["num_markers"]
    ...
    def test_workspace_broadcast_follows_ignition(self, apgi_framework, random_observation):
        """
        ...
        for _ in range(100):
            ...
            if state["ignition"]["ignition_occurred"]:
                ...
                for _ in range(5):
                    next_state = apgi_framework.step(random_observation)
...
class TestResourceCleanup:
    ...
    def test_multiple_runs_without_reset(self, apgi_framework):
        """
        ...
        """
        # First run
        result1 = apgi_framework.run(duration_ms=500.0)
        time_after_first = apgi_framework.time

        # Second run (without reset)
        result2 = apgi_framework.run(duration_ms=500.0)
        time_after_second = apgi_framework.time
    ...
    def test_reset_clears_all_subsystems(self, apgi_framework, random_observation):
        """
        ...
        assert apgi_framework.time == 0.0
        assert len(apgi_framework.history["time"]) == 0
        assert apgi_framework.metabolism.current_reserves == apgi_framework.metabolism.total_budget

    def test_system_state_after_reset_is_valid(self, apgi_framework, random_observation):
        """
        ...
        apgi_framework.run(duration_ms=500.0)
        apgi_framework.reset()
        ...
        assert state is not None
        assert state["time"] == apgi_framework.timestep_ms
        assert "ignition" in state
        assert "metabolism" in state
