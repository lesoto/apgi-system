...
@pytest.mark.unit
def test_public_path_access(auth_client):
...
@pytest.mark.unit
def test_private_path_unauthorized(auth_client):
    ...
    @auth_client.app.get("/protected")
    async def protected_route(authenticated: bool = Depends(is_authenticated)):
...
@pytest.mark.unit
def test_invalid_token_format(auth_client):
...
@pytest.mark.unit
def test_expired_token(auth_client, monkeypatch):
...
@pytest.mark.unit
def test_signature_bypass_attempt(auth_client, monkeypatch):
