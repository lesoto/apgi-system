...
from apgi_framework.monitoring import PerformanceMetrics, PerformanceMonitor
...
class TestPerformanceMonitor:
    ...
    def test_disabled_monitoring(self) -> None:
        ...
        metrics = monitor.end_step(
            system_time_ms=1.0, ignition_occurred=True, free_energy=10.0, precision=1.5
        )
    ...
    def test_history_size_limit(self) -> None:
        ...
        for i in range(10):
            monitor.start_step()
            monitor.end_step(
                system_time_ms=float(i), ignition_occurred=False, free_energy=10.0, precision=1.5
            )
    ...
    def test_reset(self, monitor) -> None:
        ...
        for i in range(5):
            monitor.start_step()
            monitor.end_step(
                system_time_ms=float(i), ignition_occurred=True, free_energy=10.0, precision=1.5
            )
