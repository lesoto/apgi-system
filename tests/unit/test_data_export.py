        ...
        **Validates: Requirements 12.1, 12.3**
        ...
        try:
            ...
            for row in rows:
                ...
                assert (
                    float(row["metabolic_reserves"]) >= 0
                ), "Metabolic reserves should be non-negative"
    ...
    def test_json_export_preserves_data_types(self) -> None:
        ...
        **Validates: Requirements 12.4**
        ...
        try:
            ...
            assert isinstance(
                entry["extero_precision"], (int, float)
            ), "Precision should be numeric"
            assert isinstance(
                entry["metabolic_reserves"], (int, float)
            ), "Reserves should be numeric"
            ...
            for orig, loaded in zip(original_data, loaded_data):
                for key in orig.keys():
                    ...
                    if isinstance(orig[key], float):
                        assert (
                            abs(orig[key] - loaded[key]) < 1e-6
                        ), f"Value for '{key}' should match"
                    else:
...
class TestAnalysisReportGeneration:
    ...
    def test_analysis_report_metabolic_cost_statistics(self) -> None:
        ...
        **Validates: Requirements 12.2**
        ...
        assert (
            abs(metabolic_stats["total_consumed"] - expected_consumed) < 1e-6
