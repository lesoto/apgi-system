...
class TestsRunnerGUI:
    ...
    def setup_ui(self) -> None:
        ...
        self.run_all_button = ttk.Button(
            control_frame, text="Run All Tests", command=self.run_all_scripts
        )
        ...
        ToolTip(self.stop_button, "Stop the currently running test script")
    ...
    def run_all_scripts(self) -> None:
        ...
        def run_all() -> None:
            for i, script in enumerate(self.scripts):
                ...
                if not success:
                    self.log_output(
                        f"Test {relative_path} failed, stopping execution", self.TAG_ERROR
                    )
                    break
